...
- Mode `td`: Cross-repo Integration is explicit if both repos are relevant, even when the existing surface covers it. Every rule in `.opencode/design-rules.md` that applies to this artifact is met. Never invent what a rule says to collect from the user.
...
- **`td`** — Solution Brief (authoritative), WWW/Pitch, resolved `investigate` reports, data collected under `.opencode/design-rules.md`, template at `.opencode/templates/technical-design-template.md`. When revising: prior draft, user decisions, and any `sync: done` decisions (settled — preserve them).
